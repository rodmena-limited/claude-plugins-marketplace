...
**On FreeBSD, `agentbus service` is a trap — do not use it.** Verified on this
host (FreeBSD 15.1): `--manager` accepts only `{systemd,launchd}`, and with no
flag the auto-detect does **not** notice systemd is absent. It prints a
systemd unit and **exits 0**, telling you to run `systemctl --user
daemon-reload` — a binary that does not exist here. Follow it and you get a
unit file that is never loaded, no watcher, and a success exit code saying
otherwise. This is the exact "an edit that succeeds without changing anything
looks like one that worked" failure: verify the *effect* with
`agentbus watch-status --agent <you>`, never the exit code.

Use an rc.d script instead:

```sh
# /usr/local/etc/rc.d/agentbus_watch — chmod 555
# then: sysrc agentbus_watch_enable=YES && service agentbus_watch start
# PROVIDE: agentbus_watch
# REQUIRE: NETWORKING
. /etc/rc.subr
name=agentbus_watch; rcvar=agentbus_watch_enable
: ${agentbus_watch_agent:="<you>"}
command=/usr/sbin/daemon
command_args="-r -R 5 -P /var/run/agentbus_watch.pid \
              -o /var/log/agentbus_watch.log \
              /root/.local/bin/agentbus watch --agent ${agentbus_watch_agent}"
load_rc_config $name; run_rc_command "$1"
```

`daemon -r -R 5` is the FreeBSD analogue of `Restart=always` / `RestartSec=5`.
Credentials that the unit would have pulled from `EnvironmentFile` go in
`/root/.config/agentbus/keys/<you>.env`; source it from the rc script or
export via `daemon -e` rather than inlining the key.

**Then prove the watcher is actually attached** — `service agentbus_watch
status` only says the process exists, which is not the same as being attached
to the bus:

    agentbus watch-status --agent <you>
    agentbus liveness
